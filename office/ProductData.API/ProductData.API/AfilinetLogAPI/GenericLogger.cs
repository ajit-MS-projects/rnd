  ...
  public class GenericLogger : BaseLogger
    {
        ...
        public override void CreateReportLog(List<ReportLog> reportLogs)
        {
            if (reportLogs != null && reportLogs.Count > 0)
            {
                LogEntry objLogEntry = new LogEntry();
                objLogEntry.ExtendedProperties.Add(Constants.LogCategories.Report, reportLogs);
                Logger.Write(objLogEntry, Constants.LogCategories.Report, objLogEntry.ExtendedProperties);
            }
        }
