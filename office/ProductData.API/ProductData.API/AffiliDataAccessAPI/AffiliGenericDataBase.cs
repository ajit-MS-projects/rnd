...
namespace Affilinet.Data.Access
{
    ...
    /// </summary>
    public class AffiliGenericDataBase : IAffiliDatabase
    {
        private Database _LibraryDataBase;
        ...
        /// </summary>
        public Database LibraryDataBase { get { return _LibraryDataBase; } }
        /// <summary>
        /// Is set to true automatically if multiple comaands are set up, one needs to set to false to use single command on same instance of AffiliGenericDataBase.
        /// </summary>
       ...
       /// </summary>
       /// <param name="dbConnectionStringId">Enum representing one of Affilinet Databases.</param>
        public AffiliGenericDataBase(DatabaseConnectionsEnum dbConnectionStringId)
        {
            _LibraryDataBase = DatabaseFactory.CreateDatabase(GetDB_Connection(dbConnectionStringId));
        ...
        {
            IsValidServer();
            Command = new System.Data.SqlClient.SqlCommand(sql) {CommandTimeout = 120};
            //Command = LibraryDataBase.GetStoredProcCommand(spName);
        }
        ...
        public void SetupCommand(string spName)
        {
            IsValidServer();
            Command = LibraryDataBase.GetStoredProcCommand(spName);
        ...
        /// </summary>
        /// <param name="spName">Stored procedure name</param>
        public void SetupMultipleCommands(string spName)
        {
            IsValidServer();
            MultipleCommands = true;
            DbCommand cmd = LibraryDataBase.GetStoredProcCommand(spName);
        ...
        private string GetDB_Connection(DatabaseConnectionsEnum dbConnect)
        {
            switch (dbConnect)
            {
                case DatabaseConnectionsEnum.LocalCache:
                    return Constants.DBConnections.LocalCache;
                case DatabaseConnectionsEnum.ProductData:
                    return Constants.DBConnections.ProductData;
                case DatabaseConnectionsEnum.Logging:
                    return Constants.DBConnections.Logging;
                case DatabaseConnectionsEnum.ProductDataLiveReadOnly:
                    return Constants.DBConnections.ProductDataLiveReadOnly;
                case DatabaseConnectionsEnum.Affilinet:
                    return Constants.DBConnections.Affilinet;
                case DatabaseConnectionsEnum.AdminUser:
                    return Constants.DBConnections.AdminUser;
                case DatabaseConnectionsEnum.LocalMachine:default:
                    return Constants.DBConnections.LocalMachine;
            }
        }
        ...
        private void IsValidServer()
        {
            ...
            String serverName = Utilities.GetAppSettingValue(Constants.AppSettings.DbServerName);
            String isEnabled = Utilities.GetAppSettingValue(Constants.AppSettings.IsDbServerNameDependencyEnabled);
            object dbServerName = DBNull.Value;
            if (serverName != null && isEnabled != null && isEnabled == "1")
            {
                serverName = serverName.ToUpper();
                Command = new System.Data.SqlClient.SqlCommand(Constants.Sqls.GetServerName) { CommandTimeout = 120 };
                do
                {
                    dbServerName = LibraryDataBase.ExecuteScalar(Command);
                    isInValid = dbServerName != DBNull.Value && dbServerName != null && dbServerName.ToString().ToUpper().Trim() != (serverName);
                    if (isInValid)
