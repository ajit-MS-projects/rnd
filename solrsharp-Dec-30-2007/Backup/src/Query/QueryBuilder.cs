...
namespace org.apache.solr.SolrSharp.Query
{
	...
	public abstract class QueryBuilder
	{
        ...
        /// </summary>
        public readonly string SOLR_SEARCH = "";
		private Dictionary<string, string> _searchkeyvalues = new Dictionary<string, string>();
        ...
        private SolrSearcher _solrsearcher;
        /// <summary>
        /// The underlying SolrSearcher used by this instance
        /// </summary>
        public SolrSearcher SolrSearcher
        {
            get { return this._solrsearcher; }
        }
        ...
        public QueryBuilder(string[] arKeys, string[] arValues, int page)
            : this()
        {
            Dictionary<string, string> searchkeyvalues = new Dictionary<string, string>();
        ...
        /// Constructs an empty, default <see cref="QueryBuilder"/> object with the
		...
		{
			if (overwrite)
				this._searchkeyvalues.Remove(parameter);
		...
		private void SetQueryUrl()
        {
            ...
            this._searchkeyvalues[QueryBuilder.SOLR_PARAM_STARTAT] = this.GetStartFromPage().ToString();
			this._searchkeyvalues[QueryBuilder.SOLR_PARAM_ROWS] = this.Rows.ToString();
            #endregion
			...
			this._searchkeyvalues[QueryBuilder.SOLR_PARAM_QUERY] = System.Web.HttpUtility.UrlEncode(queryString);

            #endregion

            #region Add debugging parameters
            this._searchkeyvalues[QueryBuilder.SOLR_DEBUG_ENABLED] = this.IsDebugEnabled.ToString().ToLower();
            if (this.ExplainOtherQuery != null)
            {
                this._searchkeyvalues[QueryBuilder.SOLR_EXPLAIN_OTHER] = System.Web.HttpUtility.UrlEncode(this.ExplainOtherQuery.ToString());
            }
            #endregion
            ...
            if (this.Sorts.Length > 0)
            {
                ...
                this._searchkeyvalues[QueryBuilder.SOLR_PARAM_QUERY] += ";" + string.Join(",", arSorts);
            }
            #endregion
			...
			{
                stringparams.Add(kvp.Key + "=" + kvp.Value);
			}
            #endregion
